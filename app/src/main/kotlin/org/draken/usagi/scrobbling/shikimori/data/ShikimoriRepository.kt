...
class ShikimoriRepository
	...
	constructor(
		...
		override val oauthUrl: String
			get() =
				"${BASE_URL}oauth/authorize?client_id=$clientId&" +
		...
		override suspend fun unregister(mangaId: Long) = db.getScrobblingDao().delete(ScrobblerService.SHIKIMORI.id, mangaId)
		...
		private fun ScrobblerManga(
			...
			isBestMatch =
				sourceTitle.equals(json.getString("name"), ignoreCase = true) ||
					json.getStringOrNull("russian")?.equals(sourceTitle, ignoreCase = true) == true,
		)

		private fun ScrobblerMangaInfo(json: JSONObject) =
			ScrobblerMangaInfo(
				id = json.getLong("id"),
				name = json.getString("name"),
				cover = json.getJSONObject("image").getString("preview").toAbsoluteUrl(DOMAIN),
		...
		private fun ShikimoriUser(json: JSONObject) =
			ScrobblerUser(
				id = json.getLong("id"),
